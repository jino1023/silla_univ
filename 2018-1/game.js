					...
					while(starting<this.limit.right){
						g.forEach(function(child){
							...
							if(c.x==starting && c.y==row){
								child.kill();
								storemove.push(child);
							}
						},this);
						starting++;
					}
					while(storemove.length>ind){
						g.remove(storemove[ind]);
						ind++;
					}
				}
			}
...
//제거한 row의 수가 10줄이 되는 경우 level을 1증가 시키고 떨어지는 속도를 증가시킴
		if(this.totalRemovedRows>9){
    ...
    createTile : function(){	
		...
		//그렇지 않을 경우 -> 게임오버 -> 다시 메뉴로 돌아가도록 설정
		else{
